...
- **Critical raw-feature finding:** `month × days_to_christmas` correlated at **r = −0.998** — the two raw columns were near-perfect mirror images of each other, adding multicollinearity with zero new information, and neither captured the real nonlinear shape of holiday demand. Both were dropped and replaced with `xmas_proximity` (a steep 1/(days+1) curve) plus cyclic `month_sin`/`month_cos` — the redesigned pair correlates at a manageable r=0.503 with every VIF under the 10 threshold (§3.2 of the report)
